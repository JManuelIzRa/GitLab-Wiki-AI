...
async def setup_repo_with_qdrant_data():
    ...
    async with AsyncSessionLocal() as session:
        ...
        session.add(WikiPage(
            repository_id=repo.id, slug="overview", title="Overview", order=0,
...
def main():
    ...
    with patch.object(VectorStore, "__init__", patched_init), \
        ...
        assert "createUser" in source["content"], "el contenido del extracto debe ser código real, no solo el nombre del archivo"
        assert isinstance(source["start_line"], int) and isinstance(source["end_line"], int)
        assert isinstance(source["score"], float)

        print("\n✅ Contrato HTTP /chat devuelve extractos de código reales junto a la respuesta")
