...
async def _index_repository(session: AsyncSession, job: IndexJob, repo: Repository,
    ...
    client = GitLabClient(base_url=gitlab_url, private_token=private_token)
    ...
    # Esto es lo que más importa ahorrar en repos grandes o cuando el usuario solo quiere
    ...
    # --- 4. Leer README y manifiestos de dependencias (contenido real) ---
    ...
    for manifest_path in structure.dependency_manifests[:10]:
        content = await client.get_file_content(project.id, manifest_path, target_branch)
        if content:
            manifest_snippets.append(FileSnippet(path=manifest_path, content=content))

    generator = WikiGenerator()

    # Borrar páginas previas de este repo (re-indexado limpio)
    old_pages = (await session.execute(select(WikiPage).where(WikiPage.repository_id == repo.id))).scalars().all()
    for p in old_pages:
    ...
    for module in top_modules:
        ...
        for file_path in module.sample_files[:SAMPLE_FILES_PER_MODULE]:
            content = await client.get_file_content(project.id, file_path, target_branch)
            if content:
                snippets.append(FileSnippet(path=file_path, content=content))
    ...
    # --- 9. Lectura de archivos de código (una sola pasada, reutilizada por Qdrant y el grafo) ---
    ...
    # --- 10. Grafo de dependencias entre módulos (análisis estático puro, sin IA ni red adicional) ---
...
async def _read_code_files(client: GitLabClient, project_id: str, branch: str, structure) -> dict[str, str]:
    ...
    reconocidas). Centralizado aquí porque tanto el embedding para Qdrant como el grafo
    de dependencias necesitan exactamente el mismo conjunto de archivos — evita una
    segunda pasada de lecturas de red para lo mismo.
    """
    code_paths = [
